  ...
  describe('extractParams', () => {
    it('should extract simple parameters', () => {
      ...
      expect(params).toEqual([
        { name: 'a', type: 'any', isRest: false, hasDefault: false, isParamProperty: false, optional: false },
        { name: 'b', type: 'any', isRest: false, hasDefault: false, isParamProperty: false, optional: false }
      ]);
    });

    it('should handle default parameters', () => {
      const node = { params: [
        { type: 'AssignmentPattern', left: { type: 'Identifier', name: 'a' }, right: { type: 'Literal', value: 5 } },
        { type: 'AssignmentPattern', left: { type: 'Identifier', name: 'b' }, right: { type: 'Literal', value: 'test' } }
      ] };
      const params = extractParams(node);
      expect(params).toEqual([
        { name: 'a', type: 'any', isRest: false, hasDefault: true, defaultValue: '5', isParamProperty: false, optional: false },
        { name: 'b', type: 'any', isRest: false, hasDefault: true, defaultValue: '"test"', isParamProperty: false, optional: false }
      ]);
    });

    it('should handle rest parameters', () => {
      const node = { params: [
        { type: 'RestElement', argument: { name: 'args' } }
      ] };
      const params = extractParams(node);
      expect(params).toEqual([
        { name: '...args', type: 'Array<any>', isRest: true, hasDefault: false, optional: false }
      ]);
    });
    ...
    it('should extract destructured, rest, and parameter property parameters correctly', () => {
      ...
      const node = {
        params: [
          ...
          {
            type: 'ObjectPattern',
            typeAnnotation: { typeAnnotation: { type: 'TSTypeReference', typeName: { name: 'BarType' } } },
            properties: [
              { type: 'Property', key: { name: 'a' }, value: { name: 'a' } },
              { type: 'Property', key: { name: 'b' }, value: { name: 'b' } }
            ]
          ...
          {
            type: 'RestElement',
            argument: { name: 'args', typeAnnotation: { typeAnnotation: { type: 'TSArrayType', elementType: { type: 'TSNumberKeyword' } } } }
          }
        ]
      };
